...
public class Quiz extends JFrame implements ActionListener{  // action listener come from awd event pack is a interface
    ...
    // action listenet use for applying event on button

    //2d string type array
    String questions[][] = new String[10][5];  //no of questions 10 in a row question and their 4 ans and 1 que itself so 5 col
    String answers[][] = new String[10][2];  //for storing questions ans 10 que ans
    String useranswers[][] = new String[10][1]; //array for tracking the ans is given by uset or not

    //to declare the variable global to explore the scope of variable out of this function

    JLabel qno, question;  //jbutton class
    ...
    public static int count = 0; //initial value 0
    public static int score = 0;

    String name;  //declaring globaly
    Quiz(String name){

        this.name = name;  //starting globali
...
//by seleting one ans from many  ans for this we use jradio button like option button
    ...
    public void actionPerformed(ActionEvent ae) {  //for overriding un implement method action perform
        if (ae.getSource() == next) {
             //when next button has called we have to do
            repaint();
            opt1.setEnabled(true);  //enabling the option of 50 50 lifeline disabled option
